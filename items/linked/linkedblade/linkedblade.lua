...
function MeleeCombo:init()
  ...
  world.sendEntityMessage(activeItem.ownerEntityId(), "az-link_init")
...
function MeleeCombo:update(dt, fireMode, shiftHeld)
  ...
  if not self.hasUpdated then self.hasUpdated = true end
...
function MeleeCombo:uninit()
  sb.logInfo("linkedblade ability: uninit")

  -- okay I have no idea why but my weapon.lua uninits on init, so... 
  --if self.hasUpdated then world.sendEntityMessage(activeItem.ownerEntityId(), "az-link_uninit") end
