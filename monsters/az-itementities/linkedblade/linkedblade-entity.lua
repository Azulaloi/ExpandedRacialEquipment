...
function init()
  ...
  self.lifeTimer = config.getParameter("duration")
  ...
  message.setHandler("return", function(_, _)
      sb.logInfo("linkedblade-entity: return")
	  despawn()
	end)
end
...
function update(dt)
  if not world.entityExists(self.parentEntity) then
	sb.logInfo("linkedblade-entity: parent no existo")
    despawn()
  end

  --self.lifeTimer = math.max(0, self.lifeTimer - dt)
...
function despawn()
  sb.logInfo("linkedblade-entity: despawning")
...
function uninit()
	sb.logInfo("az-linkedblade-entity: uninit")
	world.sendEntityMessage(self.parentEntity, "az-linked_entity-uninit", entity.id())
end