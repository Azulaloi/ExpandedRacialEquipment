...
function init()
  ...
  message.setHandler("setEnraged", function(_,_,bool)
    self.enraged = bool
  end)
  message.setHandler("tameInter", function (_, _, ...)
    return tameInter(...)
  end)

  self.despawnTimer = 5.0
  self.enraged = false
  spawnParams = config.getParameter("spawnerParams")
end
...
function update(dt)
  if self.targetId and world.entityExists(self.targetId) then
    ...
    if self.enraged == true then
      mcontroller.controlApproachVelocity(vec2.mul(toTarget, speed * enragedMult), mcontroller.baseParameters().airForce)
      else
...
function tameInter()
  if self.enraged == true and not self.tamed then
    local pos = mcontroller.position()
    self.tamed = true
    despawn()
    local id = world.spawnMonster("erchiusghostcaptured", pos, spawnParams)
    -- world.sendEntityMessage("id", "applyStatusEffect", "invisible")
    return id
  end
  return nil
end